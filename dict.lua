...
PinyinNext.Dict.PinyinToCharacters = {
  a = "阿啊呵腌嗄吖锕",
...
---@type table<string, string[]>
PinyinNext.Dict.CharacterToPinyins = {}
local delay = 0
for pinyin, characters in pairs(PinyinNext.Dict.PinyinToCharacters) do
  delay = delay + 1
  C_Timer.After(delay * 0.1, function()
    for i = 1, PinyinNext.Utils.Utf8Len(characters) do
      local character = PinyinNext.Utils.Utf8Sub(characters, i, 1)
      if not PinyinNext.Dict.CharacterToPinyins[character] then
        PinyinNext.Dict.CharacterToPinyins[character] = {}
      end
      table.insert(PinyinNext.Dict.CharacterToPinyins[character], pinyin)
    end
  end)
end
delay = delay + 1
C_Timer.After(delay * 0.1, function()
  EventRegistry:TriggerEvent("PinyinNext.GetPyTableCompleted")
end)